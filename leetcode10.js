 ...
 * so we turn to dynamic programming, and record both states: move forward or stay still
 * 代码:返回结果是DP二维表的最后一个元素,代表两个字符串的指针都走到最后一位的匹配状态
 * code: the result is the last element in the DP table, which represents the matching state when both pointers reach the end
